...
function moment(b: MomentBuild): DetectedMoment {
  return {
    ...
    focusRegion: {
      x: Math.max(0, Math.min(1, b.x - FOCUS_REGION_HALF)),
      y: Math.max(0, Math.min(1, b.y - FOCUS_REGION_HALF)),
      width: FOCUS_REGION_HALF * 2,
      height: FOCUS_REGION_HALF * 2,
    },
    ...
    recommendedIntensity: Math.min(1, b.confidenceScore + 0.05),
...
export function momentsFromEvents(
  ...
  const { duration, cursorIntent } = opts;
  ...
  // aren't double-counted by the click loop.
  for (const dc of dblclicks) {
    const [s, e] = clampWindow(dc.t - CLICK_PRE_S, dc.t + DBLCLICK_DURATION - CLICK_PRE_S, duration);
    candidates.push(
      moment({
        id: `mom_evt_${dc.id}`,
        startTime: s,
        ...
        confidenceReason: `0.97 — real double-click at ${dc.t.toFixed(2)}s`,
        eventIds: [dc.id],
        whyEffectType: "double-click → click-highlight (3.2s reveal window)",
        targetRegionSource: "click-event",
        sourceSignals: [`dblclick@${dc.t.toFixed(2)}`],
      })
    );
  ...
  // 2. Clicks (with optional hover/hesitation boost).
  for (const c of clicks) {
    if (usedClickIds.has(c.id)) continue;
    // Look for a hover ≥600ms ending within 1.5s before the click.
    ...
    let conf = 0.92;
    let reason = `0.92 — real click at ${c.t.toFixed(2)}s`;
    let cs: ConfidenceSource = "real-click";
    let whyEffectType = "click → click-highlight (0.2s pre / 1.0s post)";
    const evIds: string[] = [c.id];
    const signals: string[] = [`click@${c.t.toFixed(2)}`];
    if (mergedHover) {
      conf = 0.96;
      reason = `0.96 — real click at ${c.t.toFixed(2)}s after ${mergedHover.durationSeconds.toFixed(2)}s hover`;
      cs = "real-hover-settle";
      whyEffectType = "hover-settle → click-highlight (settle increases confidence)";
      evIds.unshift(mergedHover.id);
      signals.unshift(`hover-settle@${mergedHover.t.toFixed(2)}+${mergedHover.durationSeconds.toFixed(2)}s`);
    }
    // Cursor-intent hesitation bumps confidence slightly more.
    if (cursorIntent && cursorIntent.sampleCount > 0) {
      const b = bucketAt(cursorIntent, c.t);
      const hes = cursorIntent.samples[b]?.hesitationScore ?? 0;
      if (hes > 0.5 && !mergedHover) {
        conf = Math.min(0.99, conf + 0.04);
        cs = "cursor-intent-hesitation";
        reason = `${conf.toFixed(2)} — real click preceded by cursor hesitation (${hes.toFixed(2)})`;
        whyEffectType = "hesitation → click-highlight (cursor slowed before click)";
        signals.push(`cursor-hesitation@${hes.toFixed(2)}`);
      }
    }
    const [s, e] = clampWindow(c.t - CLICK_PRE_S, c.t + CLICK_POST_S, duration);
    candidates.push(
      moment({
        id: `mom_evt_${c.id}`,
        startTime: s,
  ...
  // 3. Right-clicks → menu zoom.
  for (const rc of rightclicks) {
    const [s, e] = clampWindow(rc.t - CLICK_PRE_S, rc.t + CLICK_POST_S + 0.4, duration);
    candidates.push(
      moment({
        id: `mom_evt_${rc.id}`,
        startTime: s,
        ...
        confidenceReason: `0.90 — real right-click at ${rc.t.toFixed(2)}s`,
        eventIds: [rc.id],
        whyEffectType: "right-click → zoom (context menu reveal)",
        targetRegionSource: "click-event",
        sourceSignals: [`rightclick@${rc.t.toFixed(2)}`],
      })
    );
  }
