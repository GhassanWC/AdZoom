...
export function materializeExportJob(
  ...
  return {
    ...
    backend: data.backend as ExportJobDoc["backend"],
    renderedFrames: data.renderedFrames as number | undefined,
    totalFrames: data.totalFrames as number | undefined,
    fpsEstimate: data.fpsEstimate as number | undefined,
    etaSeconds: data.etaSeconds as number | undefined,
    ...
    // Parallel chunked render + diagnostics.
    renderMode: data.renderMode as ExportJobDoc["renderMode"],
    chunkCount: data.chunkCount as number | undefined,
    chunkSeconds: data.chunkSeconds as number | undefined,
    workerCount: data.workerCount as number | undefined,
    chunkParallelism: data.chunkParallelism as number | undefined,
    chunksCompleted: data.chunksCompleted as number | undefined,
    chunksFailed: data.chunksFailed as number | undefined,
    progressPercent: data.progressPercent as number | undefined,
    chunkedStartedAt: millis(data.chunkedStartedAt) ?? (data.chunkedStartedAt as number | undefined),
    chunkedCompletedAt: millis(data.chunkedCompletedAt) ?? (data.chunkedCompletedAt as number | undefined),
    mergeStartedAt: millis(data.mergeStartedAt) ?? (data.mergeStartedAt as number | undefined),
    mergeCompletedAt: millis(data.mergeCompletedAt) ?? (data.mergeCompletedAt as number | undefined),
    coldStartSeconds: data.coldStartSeconds as number | undefined,
    chunkRenderSeconds: data.chunkRenderSeconds as number | undefined,
    mergeSeconds: data.mergeSeconds as number | undefined,
    totalSeconds: data.totalSeconds as number | undefined,
    machineType: data.machineType as string | undefined,
    workerImage: data.workerImage as string | undefined,
